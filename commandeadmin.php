...
<html lang="fr">
	<head>
		...
		<title>commande admin - Shop'Pop</title>
	</head>
	<body>
		<div class="header">
				...
				<?php
					if ($_SESSION['mail']=='admin@admin.admin') {
						if(isset($_SESSION['mail'])){
							...
							$mail=$_SESSION['mail'];
							$req= $db->query("SELECT idcommande,date_commande FROM commande order by idcommande DESC");
							$db->close();
							if(!$req->num_rows ==0){
								$req-> data_seek(0);
								...
								$nom=nomCommander($numero);
								$prenom=prenomCommander($numero);
								echo "<legend>Commande numéro $numero passée le $date par $nom $prenom</legend> ";
								echo "<ul>";
								...
								echo '<p><a href="function/suppcommandeadmin.php?id='.$numero.'">Annuler</a></p>';
								echo "</fieldset>";
								}
							}
							else {
